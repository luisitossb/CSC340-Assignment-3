...
void User::displayAllPosts() const {
	auto postList = posts.toVector(); //converts posts to vector for iteration
	//iterate over each post
	for (Post* post : postList) {
		Story* storyPtr = dynamic_cast<Story*>(post); //cast post to story
		if (storyPtr != nullptr) { //display the story
			storyPtr->display();
			continue; //move to next post
		}
		Reel* reelPtr = dynamic_cast<Reel*>(post); //cast reel to story
		if (reelPtr != nullptr) { //display the reel
			reelPtr->display();
			continue; //move to next post
		}
	}
